return function(container, player, uis, rs)
	warn("[N1V1LON DEBUG] ESP widget loaded")
	local espOn = false
	local espData = {} -- [player] = {box, line, name, hp, dist}
	local espConn = nil
	...
	local function ensureDrawing(p)
		...
		d.box.Visible = false
		d.box.Thickness = 1
		d.box.Color = Color3.fromRGB(255, 100, 100)

		d.line.Visible = false
		d.line.Thickness = 1
		d.line.Color = Color3.fromRGB(255, 100, 100)
		...
		d.hp.Center = true
		d.hp.Outline = true
		d.hp.Color = Color3.fromRGB(100, 255, 100)
	...
	local function updateESP()
		...
		-- remove drawings for players that left
	...
	btn.MouseButton1Click:Connect(function()
		espOn = not espOn
		warn("[N1V1LON DEBUG] ESP toggled: " .. tostring(espOn))
		if espOn then
			espStatus.Text = "ON"
			espStatus.TextColor3 = Color3.fromRGB(60, 200, 120)
			if espConn then espConn:Disconnect() end
			espConn = rs.RenderStepped:Connect(updateESP)
			warn("[N1V1LON DEBUG] ESP enabled")
		else
			espStatus.Text = "OFF"
			espStatus.TextColor3 = Color3.fromRGB(140, 60, 60)
			if espConn then espConn:Disconnect(); espConn = nil end
			clearESP()
			warn("[N1V1LON DEBUG] ESP disabled")
		end
	end)
end
